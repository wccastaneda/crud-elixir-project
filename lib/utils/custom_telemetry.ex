defmodule CrudElixirProject.Utils.CustomTelemetry do
  alias CrudElixirProject.Utils.DataTypeUtils
  alias CrudElixirProject.Utils.CustomTelemetry
  import Telemetry.Metrics

  @service_name Application.compile_env!(:crud_elixir_project, :custom_metrics_prefix_name)
  ...
  def custom_telemetry_events() do
    :telemetry.attach("crud_elixir_project-plug-stop", [:crud_elixir_project, :plug, :stop], &CustomTelemetry.handle_custom_event/4, nil)
    :telemetry.attach("crud_elixir_project-vm-memory", [:vm, :memory], &CustomTelemetry.handle_custom_event/4, nil)
  ...
  def handle_custom_event([:crud_elixir_project, :plug, :stop], measures, metadata, _) do
    :telemetry.execute(
